...
class TabEstadisticas(ctk.CTkFrame):
    ...
    def setup_ui(self) -> None:
        ...
        """
        lbl_g = ctk.CTkLabel(self, text="Resumen de Ventas",
                             font=("Arial", 14, "bold"))
        lbl_g.pack(pady=(10, 2), anchor="w", padx=20)

        self.tree_globales = ttk.Treeview(
            self, columns=("Indicador", "Valor"), show="headings", height=4
        )
        ...
        btn_refrescar = ctk.CTkButton(
            self, text="Refrescar Auditoría", command=self.actualizar_estadisticas
        )
    ...
    def actualizar_estadisticas(self) -> None:
        ...
        if "mensaje" in estadisticas:
            self.tree_globales.insert(
                "", "end", values=("Estado", estadisticas["mensaje"])
            )
        else:
    ...
    def mostrar_ventana_ticket(self, id_ticket: int) -> None:
        ...
        for item in ticket["items"]:
            ...
            ticket_str += (
                f"{item['cantidad']:<4} {prod_nombre} " f"${item['subtotal']:.2f}\n"
            )

        ticket_str += "-" * 37 + "\n"
        ticket_str += (
            f"TOTAL FACTURADO:             " f"${ticket['total_ticket']:.2f}\n"
        )
        ticket_str += "=" * 37 + "\n"
