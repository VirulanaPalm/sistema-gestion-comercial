"""
Módulo encargado de la interfaz gráfica para la pestaña de Auditoría de Caja (Estadísticas).
Permite visualizar un resumen de indicadores globales y un historial interactivo
de las ventas agrupadas por turno de caja.
"""

import customtkinter as ctk
from tkinter import ttk
from src.logica.gestion_ventas import mostrar_estadisticas, obtener_ventas_agrupadas_por_ciclo, buscar_ticket_por_id
...
class TabEstadisticas(ctk.CTkFrame):
    ...
    def setup_ui(self) -> None:
        ...
        Returns:
            None
        """
        lbl_g = ctk.CTkLabel(self, text="Resumen de Ventas", font=("Arial", 14, "bold"))
        lbl_g.pack(pady=(10, 2), anchor="w", padx=20)

        self.tree_globales = ttk.Treeview(self, columns=("Indicador", "Valor"), show="headings", height=4)
        ...
        lbl_c = ctk.CTkLabel(self, text="Auditoría por Turnos (Doble clic en Venta para ver Ticket)", font=("Arial", 14, "bold"))
        ...
        scroll_y_ciclos = ctk.CTkScrollbar(frame_ciclos, orientation="vertical")
        scroll_y_ciclos.pack(side="right", fill="y")

        self.tree_ciclos = ttk.Treeview(frame_ciclos, columns=("Total", "Detalle"), show="tree headings", yscrollcommand=scroll_y_ciclos.set)
        self.tree_ciclos.pack(side="left", fill="both", expand=True)
        scroll_y_ciclos.configure(command=self.tree_ciclos.yview)
        ...
        ctk.CTkButton(self, text="Refrescar Auditoría", command=self.actualizar_estadisticas).pack(pady=10)
    ...
    def actualizar_estadisticas(self) -> None:
        ...
        if "mensaje" in estadisticas:
            self.tree_globales.insert("", "end", values=("Estado", estadisticas["mensaje"]))
        else:
            for clave, valor in estadisticas.items():
                self.tree_globales.insert("", "end", values=(clave, valor))
        ...
        for ciclo_id in sorted(ventas_por_ciclo.keys(), reverse=True):
            ...
            # Tomamos la fecha de la primera venta del turno para mostrarla en la carpeta
            fecha_turno = tickets[0]["fecha"]
            
            padre = self.tree_ciclos.insert("", "end", text=f"Turno Caja - {fecha_turno}", values=(f"${total_ciclo:.2f}", f"{len(tickets)} ventas"), open=True)
            for t in reversed(tickets):
                self.tree_ciclos.insert(padre, "end", text=f"Venta #{t['id_ticket']}", values=(f"${t['total_ticket']:.2f}", t['fecha']), tags=(f"ticket_{t['id_ticket']}",))
    ...
    def evento_doble_click_ticket(self, event) -> None:
        ...
        Args:
            event: El evento capturado por Tkinter.
    ...
    def mostrar_ventana_ticket(self, id_ticket: int) -> None:
        ...
        Args:
            id_ticket (int): El identificador único del ticket a mostrar.
        ...
        # Todo integrado en el recuadro negro simulando impresión
        ...
        for item in ticket["items"]:
            # Cortamos el nombre si es muy largo para que no desalinee el precio
            prod_nombre = (item['nombre_producto'][:19] + '..') if len(item['nombre_producto']) > 21 else item['nombre_producto'].ljust(21)
            ticket_str += f"{item['cantidad']:<4} {prod_nombre} ${item['subtotal']:.2f}\n"
            
        ticket_str += "-"*37 + "\n"
        ticket_str += f"TOTAL FACTURADO:             ${ticket['total_ticket']:.2f}\n"
        ticket_str += "="*37 + "\n"
        ticket_str += "        ¡Gracias por su compra!      \n"
        
        textbox_items.insert("end", ticket_str)
        textbox_items.configure(state="disabled") # Lo bloqueamos para que sea solo lectura
